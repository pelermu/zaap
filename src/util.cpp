...
const char * const BITCOIN_CONF_FILENAME = "zaap.conf";
const char * const BITCOIN_PID_FILENAME = "zaapd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "zaap" is a composite category enabling all zaap-related debug output
            if(ptrCategory->count(string("zaap"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\zaapCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\zaapCore
    // Mac: ~/Library/Application Support/zaapCore
    // Unix: ~/.zaapcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "zaapCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/zaapCore";
#else
    // Unix
    return pathRet / ".zaapcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty zaap.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override zaap.conf
